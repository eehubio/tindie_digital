                ...
                <div className="border-t border-line p-4 grid lg:grid-cols-[1fr_320px] gap-5">
                  ...
                  <div>
                    <h4 className="font-semibold text-navy text-sm mb-2">
                      Rates to {d.flag} {d.label} · {f.weightG} g
                    </h4>
                    <div className="space-y-2">
                      {zone.methods.map((m) => {
                        const cost = quoteShipping(d.zone, m, f.weightG, shipProfile);
                        const carrierId = m.id.startsWith("usps") ? "usps" : "dhl_express";
                        const busy = buying === `${f.id}:${m.id}`;
                        return (
                          <div
                            key={m.id}
                            className="flex items-center gap-3 border border-line rounded-lg px-3 py-2.5"
                          >
                            <div className="flex-1">
                              <div className="font-medium text-navy text-sm">{m.name}</div>
                              <div className="text-xs text-muted">
                                {m.eta} {m.track ? "· tracked" : "· no tracking"} · via Shippo
                              </div>
                            </div>
                            <div className="font-bold text-navy">${cost.toFixed(2)}</div>
                            <button
                              className="t-btn-primary text-sm disabled:opacity-50"
                              disabled={busy || f.status !== "unfulfilled"}
                              onClick={() => {
                                setBuying(`${f.id}:${m.id}`);
                                setTimeout(() => {
                                  const lbl = mockShippoLabel(carrierId);
                                  updateFulfillment(f.id, {
                                    status: "label_purchased",
                                    carrierId,
                                    service: m.name,
                                    tracking: lbl.tracking,
                                    labelCost: cost,
                                  });
                                  setBuying(null);
                                  showToast(`Label purchased — tracking ${lbl.tracking} written back to ${f.orderRef}`);
                                }, 900);
                              }}
                            >
