...
## 12. Shared Projects nav + mirrored spaces

Projects is a top-level section both roles see, parallel to the catalog: the seller nav's Projects now points at the same public `/projects` browse as the buyer's (the management console moved behind a role-aware "Seller console: challenges & rewards →" button on that page; buyers get "+ Publish a project" in the same spot). Cross-linking is bidirectional (product Projects tab ⟷ project component links). The two private spaces mirror each other: **buyer Library** gains "Projects on hardware you own" — other makers' builds matched against your entitlements (own projects excluded); **seller console** gains "Community projects on your listings" — buyers' builds referencing your products. Same data, two lenses: the buyer sees what their hardware can do, the seller sees the community selling for them. Persist migration bumped to v3 (merges new seed entitlements by id into pre-existing local state).

## 13. Publication governance — publish rights ≠ reward rights

Sellers never gate publication. Three lanes split by the content's relationship to MONEY, not by sentiment: **challenge entries** publish immediately and the seller judges only the REFUND against the rule snapshot (a rejected refund never unpublishes the project — the seller judges the deposit, never the speech); **independent projects** publish immediately with POST-publication platform moderation (illegal/IP/spam/dangerous misinformation — "unfavorable to the seller" is not a category the admin queue recognizes); **reward claims** are fully decoupled and seller-discretionary with audited denial reasons.

Critical-project handling chain: computed **verified-purchase badge** (anti hit-piece — unverified projects exist but are marked and never reward-eligible) → seller **right of reply, not removal** (official response block; the seed includes a thermal-issue critique where the seller's "you're right, firmware 1.3 fixes it" response demonstrates the pattern) → **factual-dispute flags** ruled by the platform with the burden of proof on the flagger and verbatim-readable rulings → abusive flaggers go to review, same posture as the challenge circuit breaker. New `/admin/projects` queue; persist migration v4 normalizes governance fields on old local state.

## 14. Merged marketplace, consolidated seller Operations, real Shippo integration

**Marketplace = home + browse.** One page: search box in the hero, category pills, full catalog, matching community projects under the results, challenge banner + latest projects below. `/search` redirects home. Buyer nav: Marketplace / Projects / My Library / Preorders — the Mfg Orders button is gone (route kept for deep links).

**Seller Operations hub (`/seller/operations`).** New Listing, Orders, Preorder, Shipping and Fulfillment merged into one page under a tab strip ordered by frequency (Orders → Fulfillment → Shipping → Preorder → New Listing). The five page components moved to `components/ops/*`; the old routes now redirect with the right tab selected, so every dashboard KPI link, packslip back-link, and bookmark keeps working. Seller nav: Dashboard / Projects / Operations / Messages / Payouts.

**Shippo, per their actual API.** `/api/shippo/shipments` and `/api/shippo/transactions` mirror `api.goshippo.com` exactly (address_from/address_to/parcels → rates[] with servicelevel tokens and CHEAPEST/BESTVALUE/FASTEST attributes; rate object_id → SUCCESS + label_url + tracking_number + tracking_url_provider). With `SHIPPO_API_KEY` set the routes proxy the real API with `ShippoToken` auth; without it they answer in the identical schema — **paste a `shippo_test_…` key in Vercel and the flow goes live with zero UI changes.** The Fulfillment tab's legacy client-side mock is replaced by the real flow (`components/ShippoFlow.tsx`): create shipment → live rate cards → buy → downloadable **4x6 PDF label** (generated server-side in mock mode, dependency-free) + carrier tracking link, with a "show API JSON" toggle exposing every request/response. Store updates to `label_purchased` on success — state-2 semantics intact. Schema smoke tests in `tests/shippo.test.ts` (38 passing).

## 15. Fulfillment paperwork buttons (were dead, now wired)

"Print packing slip" and "Download commercial invoice" on the Fulfillment tab were unbound placeholders. Fixed by reuse, not new surfaces: the packslip button routes to the existing batch renderer with `?order=<id>` (one renderer, two entries — batch and single-order reprint, any status, since reprinting a shipped order's slip is a normal support task); the invoice button hits `/api/shippo/invoice`, an 8.5x11 PDF built on the same dependency-free generator (now page-size-parametrized), carrying exporter/consignee, goods line with HS code, DDU incoterm, and the IOSS declaration — shown only for international orders. With a live Shippo key, Shippo's own `commercial_invoice_url` takes over for aggregator lanes; this route remains the fallback for manual corridors (China Post/Yanwen), which is exactly when a seller must produce their own paperwork.

## 16. Admin nav consolidated into two hubs

Nine admin items collapsed to four: **Overview / Review / Projects / Configuration**. `/admin/review` merges Sellers, Product Review and Disputes — the three queues where an admin *judges* (application in, decision with a reason out). `/admin/config` merges Payments, Logistics, Partners and IP Complaints — where the data-not-code rule lives (fee modes, reserve ratios, provider and partner records, takedowns: rows an admin edits with an audit trail, never a deploy). Project moderation stays its own item on purpose: it's report-driven, not a daily queue, and its "sentiment is not a category" posture shouldn't share a page with approval habits. Same mechanics as the seller Operations hub — components moved to `components/admin/*`, all seven old routes redirect with the right tab selected.